...
@section('contenu')
    <form enctype="multipart/form-data" class="row g-3" action="{{ route('admin.service.modifier.post', ['id' => $service->id]) }}" method="post">
        @csrf
        <div class="col-lg-8">
            <div>
                <label class="form-label" for="titre">Titre du service</label>
                <input class="form-control" id="titre" name="titre" type="text" required
                    placeholder="Entrez le titre du service" value="{{ old('titre', $service->titre) }}">
                @error('titre')
        ...
        <div class="col-lg-4">
            <div>
                ...
                @if ($service->image)
                    <a target="_blank" href="{{ Storage::url($service->image) }}">Voir l'image</a>
                @endif
                @error('image')
                    <span class="text-danger">
                        {{ $message }}
        ...
        <div class="col-12">
            <div>
                ...
                <textarea class="form-control" id="resume" name="resume" required rows="4"
                    placeholder="Saisissez un résumé du service">{{ old('resume', $service->resume) }}</textarea>
                @error('resume')
                    <span class="text-danger">
                        {{ $message }}
                    </span>
        ...
        <div class="col-12">
            <div>
                ...
                <textarea class="form-control" id="description" name="description" rows="10"
                    placeholder="Décrivez le service en détail">{{ old('description', $service->description) }}</textarea>
                @error('description')
                    <span class="text-danger">
                        {{ $message }}
                    </span>
