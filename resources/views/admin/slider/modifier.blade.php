...
@section('contenu')
    <form enctype="multipart/form-data" class="row g-3" action="{{ route('admin.slider.modifier.post', ['id' => $slider->id]) }}" method="post">
        @csrf
        <div class="col-lg-8">
            <div>
                <label class="form-label" for="titre">Titre du slider</label>
                <input class="form-control" id="titre" name="titre" type="text" required
                    placeholder="Entrez le titre du slider" value="{{ old('titre', $slider->titre) }}">
                @error('titre')
        ...
        <div class="col-lg-4">
            <div>
                ...
                @if ($slider->image)
                    <a target="_blank" href="{{ Storage::url($slider->image) }}">Voir l'image</a>
                @endif
                @error('image')
                    <span class="text-danger">
                        {{ $message }}
        ...
        <div class="col-12">
            <div>
                ...
                <textarea class="form-control" id="description" name="description" rows="10"
                    placeholder="Décrivez le slider en détail">{{ old('description', $slider->description) }}</textarea>
                @error('description')
                    <span class="text-danger">
                        {{ $message }}
                    </span>
